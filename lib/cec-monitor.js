...
/**
 * CECMonitor - manages the cec-client subprocess and emits parsed CEC bus events.
 */
class CECMonitor extends events.EventEmitter {
     ...
     * @param {boolean} [options.debug] will emit debug events on cec-client traffic
     * @param {boolean} [options.autoRestart] will restart cec-client if problem happens
     * @param {string}  [options.type] type of device to emulate, defaults to 'r' for 'recording'
     * @param {number}  [options.commandTimeout] in seconds how long to wait for an answer for a command
     * @param {string}  [options.comPort] com port to use
     * @param {number}  [options.hdmiPort] hdmi port to use
     */
    constructor(OSDName, options) {
        ...
        this.params = [
            ...
            '31',
            '-p',
            (options.hdmiPort || 1).toString(),
            this.comPort.toString(),
        ];
     ...
     *
     * @returns {number} logical address of last active source
     */
    getActiveSource() {
     ...
     *
     * @returns {string} physical address in 0.0.0.0 dot notation
     */
     ...
     *
     * @returns {null|import('node:child_process').ChildProcessWithoutNullStreams} the spawned cec-client child process, or null on failure
     */
    _initCecClient() {
        this.client = spawn('cec-client', this.params);
        if (this.client) {
            // eslint-disable-next-line no-self-assign
            this.client = /* @type import('node:child_process').ChildProcessWithoutNullStreams */ this.client;
            ...
            this.client.stdin.on('error', e => this.emit('_error', e));
            this.client.stdout.on('error', e => this.emit('_error', e));
            return this.client;
        }
        return null;
    }
     ...
     *
     * @param {string} data - line of stdout output from cec-client
     */
    _processStdOut(data) {
     ...
     *
     * @param {string} plain - stdout text from cec-client
     */
    _readPacket(plain) {
    ...
    /**
     * Process TRAFFIC line from cec-client stdout
     *
     * @param {string} plain - stdout text from cec-client
     */
     ...
     *
     * @param {object} packet - parsed CEC packet from cec-client
     */
    _processEvents(packet) {
        ...
        switch (packet.opcode) {
            ...
            case CEC.Opcode.REPORT_PHYSICAL_ADDRESS: {
                if (packet.args.length !== 3) {
                    return this.emit(
                        CECMonitor.EVENTS._ERROR,
                        'opcode command REPORT_PHYSICAL_ADDRESS with bad format address or device type',
                    );
                }
            ...
            case CEC.Opcode.REPORT_POWER_STATUS: {
                if (packet.args.length !== 1) {
                    return this.emit(
                        CECMonitor.EVENTS._ERROR,
                        'opcode command REPORT_POWER_STATUS with bad format power status',
                    );
                }
            ...
            case CEC.Opcode.ROUTING_CHANGE: {
                if (packet.args.length !== 4) {
                    return this.emit(
                        CECMonitor.EVENTS._ERROR,
                        'opcode command ROUTING_CHANGE with bad format addresses',
                    );
                }
            ...
            case CEC.Opcode.USER_CONTROL_PRESSED: {
                ...
                clearInterval(this.user_control_hold_interval_ref);
                this.user_control_hold_interval_ref = setInterval(
                    () => this.emit(CECMonitor.EVENTS._USERCONTROLHOLD, data),
                    this.user_control_hold_interval,
                );
                break;
            }
    ...
    /**
     * Send a 'tx' message on CEC bus and wait for an event like response or timeout
     *
     * @async
     ...
     * @param {string} event - event name to wait for as a response
     ...
     * @see SendMessage
     * @returns {Promise<object>} resolves with response packet, rejects if adapter not ready or event timed out
     * @throws {Error} CECAdapterNotReadyError
     * @throws {Error} CECTimeoutError
     */
     ...
     * @param {number} millisecondsToWait - timeout in milliseconds
     * @returns {Promise<object>} resolves with the response packet from the target
     */
    waitForAnswer(target, event, millisecondsToWait) {
        ...
        return new Promise((resolve, reject) => {
            ...
            timeout = setTimeout(() => {
                ...
                reject(
                    new Error(
                        `CEC monitor hasn't gotten response in some time (${millisecondsToWait} ms) from ${target}`,
                    ),
                );
            }, millisecondsToWait);
     ...
     * @param {string|number|Array<number>} [_args] Optional arguments for opcode, type depending on opcode
     * @example
     ...
     * @see cec
     * @see WriteMessage
     * @returns {Promise<boolean>} resolves with true when message is sent, rejects if adapter is not ready
     * @throws {Error} CECAdapterNotReadyError
     */
     ...
     * @returns {Promise<boolean>} resolves with true when the message is written
     */
    writeRawMessage(raw) {
        return new Promise(resolve => {
            if (this.client && this.ready) {
                try {
                    this.client.stdin.write(`${raw}\n`, 'utf8', err => {
                        if (err) {
                            throw err;
                        }
                        //data is written.
     ...
     *
     * @param {number} source - logical source address
     * @param {number} target - logical target address
     * @param {number} opcode - CEC opcode byte
     * @param {Array<number>|number|undefined} args - optional arguments for the opcode
     * @returns {Promise<boolean>} resolves with true when the message is written
     */
    async writeMessage(source, target, opcode, args) {
        const msg = `tx ${[(source << 4) + target, opcode]
            .concat(args || [])
            .map(h => `0${h.toString(16)}`.substr(-2))
            .join(':')}`;
     ...
     *
     * @returns {Promise<void>} resolves when cec-client emits the ready event
     */
    async waitForReady() {
        return new Promise(resolve => {
 ...
 * @returns {boolean} True if it matches form 0.0.0.0 otherwise false
 */
...
function isRoute(address) {
    return /^\d\.\d\.\d\.\d$/giu.test(address);
}
 ...
 * @returns {string} Physical address in . notation ie 0.0.0.0
 */
 ...
 * @returns {number} Physical address in integer CEC protocol notation
 */
 ...
 * @returns {number[]} A two-byte encoded version represented as an array
 */
 ...
 * @returns {number} parsed logical address, or the default value if invalid
 */
