/**
 * Based losely on the work of @senzil/cec-monitor but reduced a lot to work for this adapter.
...
class CECMonitor extends events.EventEmitter {
    constructor(OSDName, options) {
        ...
        this.autoRestarting = false; //flag to recognize if we are autorestarting or not.
        ...
        this.params = ['-t', this.type, '-o', this.OSDName, '-d', '31', '-p', options.hdmiport || 1, this.comPort];
    ...
    _initCecClient() {
        ...
        this.client.stdout
            .pipe(estream.split())
            .pipe(estream.map((data, cb) => { this._processStdOut(data, cb); }));
     ...
     * @param {string} data
     * @param {function} cb callback of event-stream.map
     * @private
     */
    _processStdOut(data, cb) {
    ...
    /**
     * Binds a new listener to this, waiting on for answer 'event' from 'target' for 'milisecondsToWait' or rejects with timeout.
     * @param {number} target logical address of target as number
     * @param {string} event name of event (from EVENTS array)
     * @param milisecondsToWait timeout
     * @returns {Promise<unknown>}
     */
    waitForAnswer(target, event, milisecondsToWait) {
        ...
        return new Promise((resolve, reject) => {
            ...
            timeout = setTimeout(() => {
                removeListener()
                reject(new Error(`CEC monitor hasn't gotten response in some time (${milisecondsToWait} ms) from ${target}`))
            }, milisecondsToWait);
     ...
     * @throws CECAdapterNotReadyError
     */
    async sendMessage(source, target, _opcode, args) {
        source = _parseAddress(source, this.ourLogicalAddress);
        target = _parseAddress(target, CEC.LogicalAddress.BROADCAST);

        let opcode = 0;
        if (typeof _opcode === 'string') {
            if (isHexaNumber(_opcode)){
        ...
        if (typeof args === 'string') {
            ...
            if (isRoute(args)) {
                args = routeToArgs(args);
            }
            else if (isHexaNumber(args)){
