 ...
 *
 * [key]                 - optional key of the state in stateDefinitions-Object,
 *                                               if name differs from key (because name is not suitable for ioBroker id)
 *
 * [readOnly]           - make readOnly for CEC
 *
 * [parse]             - parse CEC event to state
 *
 * [idPrefix]            - prefix id of state key with this string for subdirectories.
 *
 * [pollOpCode]          - opcode used for polling, will also create a poll-button.
 *
 * [pollArgument]        - argument for pollOpCode, if polling is more complex.
 *
 * [pollTarget]          - if polling targets a different logical address (especially for global states)
 *
 * [command] - async function that is called if state is written -> i.e. control devices.
 *
 * [isGlobal]           - belongs to the global device
 *
 * type - ioBroker type, i.e. "string", "number", "boolean", ...
 *
 * write                - make ioBroker state writable
 *
 * [read]               - make ioBroker state readable
 *
...
const stateDefinitions = {
    active: {
        //seen this session -> only then LE is valid.
        name: 'active',
        type: 'boolean',
        write: false,
    ...
    powerState: {
        ...
        parse: data => {
            return (
                data.data.val === CEC.PowerStatus.ON || data.data.val === CEC.PowerStatus.IN_TRANSITION_STANDBY_TO_ON
            );
        },
        role: 'switch.power',
    ...
    maxCecVersionSupported: {
        //usually handled by libPulse -> but some devices do not support cec1.4 and report with feature abort.. this seems to lead to endless loop in some defices, they will request info over and over again.
        //so the idea is to handle this myself and report a lower version. Let's see how that works out. ;-)
        //command would be: b1:9f
        name: 'maxCecVersionSupported',
        type: 'string',
        role: 'info.version',
        write: false,
        idPrefix: 'info',
    ...
    menuStatus: {
        ...
        command: async function (value, device, cec) {
            if (device.logicalAddress === CEC.LogicalAddress.TV) {
                await cec.sendMessage(
                    null,
                    device.logicalAddress,
                    CEC.Opcode.MENU_REQUEST,
                    value ? CEC.MenuRequestType.ACTIVATE : CEC.MenuRequestType.DEACTIVATE,
                );
            }
            await cec.sendMessage(
                CEC.LogicalAddress.TV,
    ...
    systemAudio: {
        ...
        command: async function (_value, _device, cec) {
            ...
            return cec.sendCommand(
                cec.getActiveSource(),
                CEC.LogicalAddress.AUDIOSYSTEM,
                CEC.Opcode.SYSTEM_AUDIO_MODE_REQUEST,
                CECMonitor.EVENTS.SET_SYSTEM_AUDIO_MODE,
                cec.getActiveSourcePhysical(),
            );
        },
    ...
    arc: {
        ...
        command: async function (value, _device, cec) {
            ...
            return cec.sendMessage(
                CEC.LogicalAddress.AUDIOSYSTEM,
                CEC.LogicalAddress.TV,
                value ? CEC.Opcode.START_ARC : CEC.Opcode.END_ARC,
            );
        },
        isGlobal: true,
    },
    ...
    'active-source': {
        ...
        command: async function (value, device, cec) {
            ...
            if (value.length === 1) {
                //allow single numbers for hdmi port.
                value += '.0.0.0';
            }
    ...
    volumeDown: {
        ...
        command: async function (_value, _device, cec) {
            await cec.sendMessage(
                ...
                CEC.LogicalAddress.AUDIOSYSTEM,
                CEC.Opcode.USER_CONTROL_PRESSED,
                CEC.UserControlCode.VOLUME_DOWN,
            );
            await cec.sendMessage(null, CEC.LogicalAddress.AUDIOSYSTEM, CEC.Opcode.USER_CONTROL_RELEASE);
        },
        isGlobal: true,
    ...
    'osd-message': {
        ...
        command: async function (value, _device, cec) {
            if (value) {
                const param = [CEC.DisplayControl.DISPLAY_FOR_DEFAULT_TIME].concat(
                    value.split('').map(s => s.charCodeAt(0)),
                );
    ...
    'osd-message-clear': {
        ...
        command: async function (_value, _device, cec) {
            await cec.sendMessage(
                null,
                CEC.LogicalAddress.TV,
                CEC.Opcode.SET_OSD_STRING,
                CEC.DisplayControl.CLEAR_PREVIOUS_MESSAGE,
            );
        },
        isGlobal: true,
    },
