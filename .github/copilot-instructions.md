...
This adapter (**ioBroker.cec2**) integrates with HDMI CEC (Consumer Electronics Control) to monitor and control multimedia devices connected via HDMI. It spawns and manages a `cec-client` subprocess (from the `cec-utils` Linux package) to communicate with the HDMI CEC bus. The adapter creates ioBroker states for every discovered CEC device and supports power control, volume management, input switching, active source selection, button presses, and raw CEC command sending.

## Adapter-Specific Context

- **Adapter Name**: cec2
- **Primary Function**: Monitor and control HDMI CEC multimedia devices (TVs, audio systems, playback devices, set-top boxes) via the local HDMI CEC bus
- **Key Dependencies**: `cec-utils` OS package (provides `cec-client` binary); `@iobroker/adapter-core` for ioBroker integration
- **Communication**: Purely local — no internet/cloud API involved; communicates with hardware via `/dev/vchiq` (or standard CEC adapter)
- **Configuration Requirements**: OSD name (reported to CEC bus), device type (logical address type), poll interval, power state polling toggle, prevent-unnamed-devices toggle
...
### cec-client subprocess management

The adapter spawns `cec-client` as a child process to communicate with the HDMI CEC bus. Key patterns:

- Always terminate the `cec-client` process gracefully in the `onUnload()` method to avoid zombie processes
- Use `readline` to parse cec-client's line-by-line stdout output
- Implement automatic restart of `cec-client` when it exits unexpectedly (with a delay to avoid tight loops)
- Parse CEC bus addresses carefully: logical address (0–15) and physical address (e.g. `1.0.0.0`)
- Handle the case where `cec-client` is not installed (`ENOENT` error) with a user-friendly error message

**cec-client cleanup example:**
```javascript
onUnload(callback) {
    try {
        if (this.cecProcess) {
            this.cecProcess.removeAllListeners();
...
### CEC Device State Management

- Device identity is based on OSD name (not logical address, which can change)
- Fall back to physical address as device ID when OSD name is not yet available
- Track `info.active` state — set to `false` when a device's logical address is reassigned
- Set `info.connection` state on the adapter instance to reflect cec-client connectivity

### Raw CEC Command Handling

- The `raw-command` state allows users to send arbitrary commands to cec-client (e.g. `scan`, `tx 10:36`)
- Validate and sanitize raw commands before passing to cec-client stdin
- Log all raw commands at debug level for troubleshooting

### Testing Without Hardware

- Unit tests must mock the `cec-client` subprocess entirely (use `proxyquire` or `sinon` stubs)
- Provide fixture data files representing typical cec-client output lines for parsing tests
- Integration tests that require actual CEC hardware should be clearly marked and excluded from standard CI runs
